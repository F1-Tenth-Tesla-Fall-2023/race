...
arrow2_marker_pub = rospy.Publisher("/arrow_marker2", Marker, queue_size = 2)
prev_range = 0.0
...
def callback2(data):
    ...
    # Convert steering Euler angle (yaw) to quaternion
    example_steering = data.steering_angle
    example_steering = math.radians(example_steering)
    quaternion = quaternion_from_euler(0,0,example_steering) # args: Roll, Pitch, Yaw
    sphere_marker.pose.orientation.x = quaternion[0]
    sphere_marker.pose.orientation.y = quaternion[1]
    sphere_marker.pose.orientation.z = quaternion[2]
    ...
    arrow2_marker_pub.publish(sphere_marker)
   
if __name__=='__main__':
    rospy.init_node("rviz_test", anonymous=False)   
    sub = rospy.Subscriber("/car_8/scan",LaserScan, callback)
    sub = rospy.Subscriber("/car_8/offboard/command",AckermannDrive, callback2)
    rospy.spin()
